...
import tech.stackable.nifi.processors.iceberg.UnmatchedColumnBehavior;

/**
 * This class is responsible for schema traversal and data conversion between NiFi and Iceberg
...
public class IcebergRecordConverter {
  ...
  public IcebergRecordConverter(
      ...
      ComponentLog logger) {
    this.converter =
        (DataConverter<Record, GenericRecord>)
            IcebergSchemaVisitor.visit(
                schema,
  ...
  private static class IcebergSchemaVisitor
    ...
    public static DataConverter<?, ?> visit(
        Schema schema,
      ...
      return visit(
          schema,
          new RecordTypeWithFieldNameMapper(schema, recordDataType),
          new IcebergSchemaVisitor(),
          new IcebergPartnerAccessors(schema, fileFormat, unmatchedColumnBehavior, logger));
    }

    @Override
    public DataConverter<?, ?> schema(
        Schema schema, DataType dataType, DataConverter<?, ?> converter) {
      return converter;
    }

    @Override
    public DataConverter<?, ?> field(
        Types.NestedField field, DataType dataType, DataConverter<?, ?> converter) {
      ...
      if (type.typeId() != null) {
        switch (type.typeId()) {
          ...
          case DECIMAL:
            ...
            return new GenericDataConverters.BigDecimalConverter(
    ...
    @Override
    public DataConverter<?, ?> struct(
        Types.StructType type, DataType dataType, List<DataConverter<?, ?>> converters) {
    ...
    @Override
    public DataConverter<?, ?> list(
        Types.ListType listTypeInfo, DataType dataType, DataConverter<?, ?> converter) {
      return new GenericDataConverters.ArrayConverter<>(
          converter, ((ArrayDataType) dataType).getElementType());
    }

    @Override
    public DataConverter<?, ?> map(
        Types.MapType mapType,
        DataType dataType,
        DataConverter<?, ?> keyConverter,
        DataConverter<?, ?> valueConverter) {
      return new GenericDataConverters.MapConverter<>(
          keyConverter,
  ...
  public static class IcebergPartnerAccessors
    ...
    public DataType fieldPartner(DataType dataType, int fieldId, String name) {
      Validate.isTrue(
          dataType instanceof RecordTypeWithFieldNameMapper,
          String.format("Invalid record: %s is not a record", dataType));
      final RecordTypeWithFieldNameMapper recordType = (RecordTypeWithFieldNameMapper) dataType;
      ...
      if (mappedFieldName.isEmpty()) {
        if (UnmatchedColumnBehavior.WARNING_UNMATCHED_COLUMN.equals(unmatchedColumnBehavior)) {
          if (logger != null) {
            logger.warn(
                "Cannot find field with name '{}' in the record schema, using the target schema for datatype and a null value",
                name);
          }
        }
        ...
        if (schemaField.isRequired()) {
          ...
          throw new IllegalArgumentException(
              "Iceberg requires a non-null value for required fields, field: "
                  + schemaField.name()
                  + ", type: "
                  + schemaFieldType);
        }
      ...
      // If the actual record contains a nested record then we need to create a
      // RecordTypeWithFieldNameMapper wrapper object for it.
      if (fieldType instanceof RecordDataType) {
        return new RecordTypeWithFieldNameMapper(
            new Schema(schema.findField(fieldId).type().asStructType().fields()),
            (RecordDataType) fieldType);
      }

      // If the field is an Array, and it contains Records then add the record's iceberg schema for
      // creating RecordTypeWithFieldNameMapper
      if (fieldType instanceof ArrayDataType
          && ((ArrayDataType) fieldType).getElementType() instanceof RecordDataType) {
      ...
      // If the field is a Map, and it's value field contains Records then add the record's iceberg
      // schema for creating RecordTypeWithFieldNameMapper
      if (fieldType instanceof MapDataType
          && ((MapDataType) fieldType).getValueType() instanceof RecordDataType) {
        return new MapTypeWithIcebergSchema(
            new Schema(
                schema.findField(fieldId).type().asMapType().valueType().asStructType().fields()),
    ...
    public DataType mapValuePartner(DataType dataType) {
      Validate.isTrue(
          dataType instanceof MapDataType, String.format("Invalid map: %s is not a map", dataType));
      ...
      if (mapType instanceof MapTypeWithIcebergSchema) {
        ...
        return new RecordTypeWithFieldNameMapper(
            typeWithSchema.getValueSchema(), (RecordDataType) typeWithSchema.getValueType());
      }
    ...
    public DataType listElementPartner(DataType dataType) {
      Validate.isTrue(
          ...
          String.format("Invalid array: %s is not an array", dataType));
  ...
  /**
   * Since the {@link RecordSchema} stores the field name and value pairs in a HashMap it makes the
   * retrieval case-sensitive, so we create a name mapper for case-insensitive handling.
   */
  private static class RecordTypeWithFieldNameMapper extends RecordDataType {
    ...
    RecordTypeWithFieldNameMapper(Schema schema, RecordDataType recordType) {
      ...
      final Map<String, String> lowerCaseMap =
          recordType.getChildSchema().getFieldNames().stream()
      ...
      schema
          .columns()
          .forEach(
              (s) -> this.fieldNameMap.put(s.name(), lowerCaseMap.get(s.name().toLowerCase())));
    }
  ...
  /**
   * Data type for Arrays which contains Records. The class stores the iceberg schema for the
   * element type.
   */
  ...
  /**
   * Data type for Maps which contains Records in the entries value. The class stores the iceberg
   * schema for the value type.
   */
