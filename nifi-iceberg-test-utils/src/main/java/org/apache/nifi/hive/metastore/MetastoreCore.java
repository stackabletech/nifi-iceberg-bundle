...
class MetastoreCore {
  ...
  public void initialize(Map<String, String> configOverrides)
      throws IOException,
          TException,
    ...
    metaStoreClient.createDatabase(
        new Database(DATABASE_NAME, "description", getDBPath(), new HashMap<>()));
  }
  ...
  private HiveConf hiveConf(int port, Map<String, String> configOverrides) throws IOException {
    ...
    hiveConf.set(
        HIVE_TXN_MANAGER.getVarname(), "org.apache.hadoop.hive.ql.lockmgr.DummyTxnManager");
  ...
  private TServer thriftServer(Map<String, String> configOverrides)
    ...
    final HiveMetaStore.HMSHandler baseHandler =
        new HiveMetaStore.HMSHandler("new db based metaserver", hiveConf);
  ...
  private void setupDB(String dbURL) throws SQLException, IOException {
    ...
    final InputStream inputStream =
        getClass().getClassLoader().getResourceAsStream("hive-schema-4.0.0-alpha-2.derby.sql");
