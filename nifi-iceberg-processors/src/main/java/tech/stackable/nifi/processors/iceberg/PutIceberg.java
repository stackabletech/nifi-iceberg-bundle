...
import static tech.stackable.nifi.processors.iceberg.IcebergUtils.getDynamicProperties;
...
import tech.stackable.nifi.processors.iceberg.writer.IcebergTaskWriterFactory;
...
@CapabilityDescription(
    "This processor uses Iceberg API to parse and load records into Iceberg tables. "
...
@DynamicProperty(
    name =
        "A custom key to add to the snapshot summary. The value must start with 'snapshot-property.' prefix.",
    value = "A custom value to add to the snapshot summary.",
    expressionLanguageScope = ExpressionLanguageScope.FLOWFILE_ATTRIBUTES,
    description =
        "Adds an entry with custom-key and corresponding value in the snapshot summary. The key format must be 'snapshot-property.custom-key'.")
@WritesAttributes({
  @WritesAttribute(
      attribute = "iceberg.record.count",
...
public class PutIceberg extends AbstractIcebergProcessor {
  ...
  static final PropertyDescriptor FILE_FORMAT =
      new PropertyDescriptor.Builder()
          .name("file-format")
          .displayName("File Format")
          .description(
              "File format to use when writing Iceberg data files."
  ...
  static final PropertyDescriptor MAXIMUM_FILE_SIZE =
      new PropertyDescriptor.Builder()
          .name("maximum-file-size")
          .displayName("Maximum File Size")
          .description(
              "The maximum size that a file can be, if the file size is exceeded a new file will be generated with the remaining data."
  ...
  static final PropertyDescriptor NUMBER_OF_COMMIT_RETRIES =
      new PropertyDescriptor.Builder()
  ...
  static final PropertyDescriptor MINIMUM_COMMIT_WAIT_TIME =
      new PropertyDescriptor.Builder()
  ...
  static final PropertyDescriptor MAXIMUM_COMMIT_WAIT_TIME =
      new PropertyDescriptor.Builder()
  ...
  static final PropertyDescriptor MAXIMUM_COMMIT_DURATION =
      new PropertyDescriptor.Builder()
  ...
  static final Relationship REL_SUCCESS =
      new Relationship.Builder()
          .name("success")
          .description(
              "A FlowFile is routed to this relationship after the data ingestion was successful.")
          .build();
            ...
            (subject, input, context) -> {
              ...
              if (subject.startsWith(ICEBERG_SNAPSHOT_SUMMARY_PREFIX)) {
                ...
                builder
                    .valid(false)
                    .explanation(
                        "Dynamic property key must begin with '"
                            + ICEBERG_SNAPSHOT_SUMMARY_PREFIX
                            + "'");
              }
  ...
  @Override
  public void doOnTrigger(ProcessContext context, ProcessSession session, FlowFile flowFile)
    ...
    try (final InputStream in = session.read(flowFile);
      ...
      final IcebergRecordConverter recordConverter =
          new IcebergRecordConverter(
              table.schema(), reader.getSchema(), format, unmatchedColumnBehavior, getLogger());
  ...
  void appendDataFiles(ProcessContext context, FlowFile flowFile, Table table, WriteResult result) {
    final int numberOfCommitRetries =
        context
            .getProperty(NUMBER_OF_COMMIT_RETRIES)
            .evaluateAttributeExpressions(flowFile)
            .asInteger();
    final long minimumCommitWaitTime =
        context
            .getProperty(MINIMUM_COMMIT_WAIT_TIME)
            .evaluateAttributeExpressions(flowFile)
            .asTimePeriod(TimeUnit.MILLISECONDS);
    final long maximumCommitWaitTime =
        context
            .getProperty(MAXIMUM_COMMIT_WAIT_TIME)
            .evaluateAttributeExpressions(flowFile)
            .asTimePeriod(TimeUnit.MILLISECONDS);
    final long maximumCommitDuration =
        context
            .getProperty(MAXIMUM_COMMIT_DURATION)
            .evaluateAttributeExpressions(flowFile)
            .asTimePeriod(TimeUnit.MILLISECONDS);
    ...
    Tasks.foreach(appender)
        .exponentialBackoff(
            minimumCommitWaitTime, maximumCommitWaitTime, maximumCommitDuration, 2.0)
  ...
  /**
   * Adds the FlowFile's uuid and additional entries provided in Dynamic properties to the snapshot
   * summary.
   *
   * @param context processor context
   ...
   */
  private void addSnapshotSummaryProperties(
      ProcessContext context, AppendFiles appender, FlowFile flowFile) {
    appender.set(
        ICEBERG_SNAPSHOT_SUMMARY_FLOWFILE_UUID, flowFile.getAttribute(CoreAttributes.UUID.key()));
  ...
  private FileFormat getFileFormat(Map<String, String> tableProperties, String fileFormat) {
    final String fileFormatName =
        fileFormat != null
            ? fileFormat
            : tableProperties.getOrDefault(DEFAULT_FILE_FORMAT, DEFAULT_FILE_FORMAT_DEFAULT);
